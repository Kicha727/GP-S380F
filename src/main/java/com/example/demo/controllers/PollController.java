...
public class PollController {
    ...
    @GetMapping("/{id}/zh")
    public String viewPollZh(@PathVariable Long id, Model model, HttpSession session) {
        Optional<Poll> pollOpt = pollRepository.findById(id);
        
        if (pollOpt.isEmpty()) {
            return "redirect:/index_zh";
        }
    ...
    @PostMapping("/{pollId}/vote_zh")
    public String voteZh(@PathVariable Long pollId, @RequestParam Long optionId, HttpSession session) {
        Long userId = (Long) session.getAttribute("userId");
        if (userId == null) {
        ...
        return "redirect:/polls/" + pollId +"/zh";
    }
    ...
    @PostMapping("/{pollId}/comment_zh")
    public String addCommentZh(@PathVariable Long pollId, @RequestParam String content, HttpSession session) {
        Long userId = (Long) session.getAttribute("userId");
        if (userId == null) {
            return "redirect:/login";
        }
    ...
    @GetMapping("/create_zh")
    public String createPollFormZh(HttpSession session, Model model) {
        Long userId = (Long) session.getAttribute("userId");
        if (userId == null) {
            return "redirect:/login_zh";
        }
    ...
    @PostMapping("/create_zh")
    public String createPollZh(@RequestParam String question, 
                            @RequestParam String option1,
                            @RequestParam String option2,
                            @RequestParam String option3,
                            @RequestParam String option4,
        ...
        return "redirect:/polls/" + poll.getId()+"/zh";
    }
    ...
    @GetMapping("/zh")
    public String listPollsZh(Model model) {
        List<Poll> polls = pollRepository.findAll();
        model.addAttribute("polls", polls);
        return "polls_zh";
    }
    ...
    @PostMapping("/{pollId}/delete_zh")
    public String deletePollZh(@PathVariable Long pollId, HttpSession session) {
        Long userId = (Long) session.getAttribute("userId");
        if (userId == null) {
            return "redirect:/login_zh";
        }
